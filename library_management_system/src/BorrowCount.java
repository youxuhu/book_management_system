...
public class BorrowCount extends JFrame implements ActionListener {
    ...
    String user;
    int flag;
    public BorrowCount(String user,int flag) {
        super("图书信息管理程序_查询借出图书信息");
        this.flag=flag;
        this.user=user;
        ...
        jF.requestFocus();
        try {
            Borrow_n_Return_Book.searchAllTxtFile();
            textArea.setText(Borrow_n_Return_Book.getreadStr());
    ...
    public static void main(String[] args){
        BorrowCount b=new BorrowCount("user",1);
    }
            ...
            if (jF1.getText().toString() != null && jF1.getText().toString().length() > 0) {
                if (jF1.getText().toString().equals("search.all")) {
                    try {
                        ...
                        // System.out.println(Borrow_n_Return_Book.getreadStr());
                    ...
                    try {
                        Borrow_n_Return_Book.searchTxtFile(jF1.getText().toString());
                        textArea.setText(Borrow_n_Return_Book.getreadStr());
                        JOptionPane.showMessageDialog(w2, "查询成功", "提示", 1);
                        // System.out.println(Borrow_n_Return_Book.getreadStr());
                    } catch (UnsupportedEncodingException e1) {
